...
calculate_pod_quantiles <- function(dose_response_data,
  ...
  interpolated_doses <- seq(min(dose_vector),
                            max(dose_vector),
                            length.out = interpolation_size)

  # Perform bootstrap
  pods <- lapply(1:resample_size, function(x) {
  ...
  pods_dose <- sapply(pods, function(x) 10^x[["log10_pod"]])
  ...
  if (spline_res) {
    spline_output <- lapply(1:resample_size, function(i) {
      data.frame(bs_index = i,
                 pods[[i]][[1]])
    })
    spline_output <- do.call("rbind", spline_output)
    spline_doses <- 10^spline_output$log10_dose
    if (any(doses == 0)) {
    ...
    spline_output <- spline_output[,c("bs_index", "dose", "log10_dose", "response_pred", "mc")]
    out <- list(
